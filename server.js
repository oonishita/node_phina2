...
function requestListener(request, response) {
  ...
  switch (extensionName) {
    ...
    case '.json':
      ...
      break;
    case '.mp3':
      readFileHandler(requestURL, 'audio/mp3', true, response);
      break;
    case '.png':
...
io.sockets.on('connection', (socket) => {
  ...
  // コントローラーからtouchDownFromControler1というデータを受信（コントローラでマウスダウンイベントが発生）
  socket.on('touchDownFromControler1', (data) => {
    socket.to(roomID).broadcast.emit('touchDownToMain1', data);
  });
  // コントローラーからtouchDownFromControler2というデータを受信（コントローラでマウスダウンイベントが発生）
  socket.on('touchDownFromControler2', (data) => {
    socket.to(roomID).broadcast.emit('touchDownToMain2', data);
  });
